...
for f in release_details[args.release]['compressed_files']:
    downloaded_file = download(
        urljoin(
            release_details[args.release]['base_url'], f),
...
decay_zip = download(DECAY_LIB[args.lib], output_path=endf_files_dir)
nfy_file = download(NFY_LIB[args.lib], output_path=endf_files_dir)
